...
3. **Subsampling stability.** Median `|H(50%) − H(100%)|` under **20% of the estimator's own full-depth between-spot SD**. ~~*and* Spearman rank correlation between the 50% and 100% values above **0.90**. The rank half is an addition: an estimator can shift level uniformly (harmless — it is monotone in the covariate of interest) or shuffle the spot ordering (fatal). The draft's single criterion cannot tell those apart.~~ Ratio form rather than absolute bits because the estimators do not share a scale. Measured at A4.
   ...
   > **Correction (post-A4, D2).** The rank half was my addition and it was wrong. Common-proportion thinning preserves the spots' relative depth ordering almost exactly, so *any* metric that is monotone in depth reproduces its own ranks — a pure `nCount` readout scores **0.9998** on all three samples, essentially the ceiling. Across the twelve (sample × estimator) cells, depth coupling and this rank score correlate at ρ = 0.881. The clause was measuring depth coupling under another name, i.e. exactly the failure mode criterion 1 exists to punish, and plug-in's 0.994 was a symptom of the defect rather than evidence of quality. **Retired**, and replaced descriptively (no threshold — inventing a gate post-hoc is what D6 forbids) by a depth-free split-half: two *independent* 50% thinnings, correlated with both partialled on the spot's full-depth total. The concern that motivated the clause was still correct, and survives on the valid measure — Chao–Shen loses 0.05–0.07 of depth-free reliability against plug-in on the shallow samples. Both original columns stay computed as `C3_Rank_Preserved_Retired` / `C3_Subsampling_Stability_As_Preregistered`.
4. **Dynamic range.** ~~**CV = SD/mean ≥ 0.02 and**~~ **IQR ≥ 0.10 bits**, per sample. The draft had no number, which makes it un-pre-registerable — "large enough to be usable" is exactly the kind of criterion that gets argued into whatever the result turned out to be. ~~Anchored on rarefaction's documented failure (CV 0.88%) at a bit over twice it. Calibration check: the plug-in baseline on sample1 sits at CV = 2.29% (SD 0.2691, mean 11.74) and IQR = 0.2249, so the floor is live but not absurd — a candidate that fails it is genuinely worse than the metric it replaces.~~
   ...
   > **Correction (post-A3, D2).** The CV half was also mine, and it is the wrong statistic. CV is ratio-scale; entropy in bits has no meaningful zero — the origin is set by the gene-universe size (log₂ K), a property of the reference (D5), not the tissue. Enlarging the universe shifts every H by a near-constant and changes the CV while changing nothing biological, so the mean is not a unit and SD/mean is not interpretable. **IQR in bits is the defensible half**; the CV half is **retired** and kept as `C4_Dynamic_Range_CV_Retired`. Under IQR alone criterion 4 passes for every estimator on every sample (minimum 0.1645, Chao–Shen on sample1) — it is a real floor against a rarefaction-style collapse, but at these thresholds it **carries no information** and separates none of the candidates.
...
Criteria 1, 2, 4 and 5(a) are read off `results/statistical_tests/<sample>_estimator_comparison.csv` (A3); criterion 3 off the A4 ladder, with the depth-free split-half reliability in `<sample>_subsampling_reliability.csv` alongside it; 5(b) at Stage C.
...
Result: **Chao–Shen removes 53–68% of the plug-in's depth bias**, and that fraction is stable across three samples and three thinning levels — a clean, reproducible finding. Miller–Madow removes 5–25%. But the figure is not the one hoped for: `ΔH` is not near zero, it is merely halved. Median signed `ΔH` at 50% depth is −0.176 / −0.163 / −0.170 bits for Chao–Shen against −0.376 / −0.490 / −0.443 for plug-in. **Criterion 3's magnitude half fails for every estimator on every sample** (3–6× over the 0.20 × SD bound) — including plug-in, so it does not separate the candidates either.

~~Chao–Shen's rank preservation on sample21 (ρ = 0.896) falls *below* the plug-in's (0.994).~~ **Retired — that comparison was an artefact; see the correction under criterion 3 above.** On the depth-free split-half replacement the cost is real but bounded: Chao–Shen 0.959 / 0.898 / 0.918 against plug-in 0.973 / 0.971 / 0.973, with a pure-depth pseudo-metric at ≈ 0 and noise at ≈ 0. The mechanism is as suspected — the correction is driven by the noisy quantity `f1/N`, so it buys a smaller level shift at some cost in spot-level precision. Incidentally, plug-in holding 0.971–0.973 depth-free reliability says it is not *purely* a depth readout even where 96–98% of its rank variance is depth-correlated, which matters for the A5 fallback branch.

Full tables in D2. The thresholds were not revised after seeing these numbers.

### A5 — Gate and commit ✅ DONE

Check the A0 criteria against A3/A4. Then one of:

- ~~**Chao–Shen passes** → commit it as the primary metric.~~ Not satisfied: criterion 1 passes on 6/16 cohort samples.
- ~~**Chao–Shen overcorrects** (sign flip, or `percent.mt` association inflates the way it did under rarefaction) → fall back to Chao–Shen-with-CWJ-coverage or Miller–Madow, which correct less aggressively.~~ Not available as written: no `nFeature` sign flip anywhere, CWJ is Chao–Shen to four decimals, and Miller–Madow *is* the plug-in once adjusted for depth (both-adjusted ρ = 0.969). Neither "corrects less aggressively" in a useful way. But the `percent.mt` half of this branch **did** partly fire — see the correction below.
- ~~**Nothing decouples adequately** → keep the plug-in estimator, and make the depth-adjusted partial correlation the headline number everywhere.~~ Trigger satisfied, prescription refuted by measurement — see below.

> **Correction (post-A5, D2). All three branches were miscalibrated, and the measurement that settles it was not in this plan.**
>
> The branches share a hidden assumption: that "correct at the inference stage" is an equivalent fallback when the metric is imperfect. **A5a measured it and it is not.** After the `log(nCount)` partial, the plug-in's residual still has spline R² = **0.597** against `log(nCount)` (median, 16/16 above 0.05); Chao–Shen's is **0.044**. Roughly 60% of what the inference-side route leaves behind is still depth. Branch 3 is not a weaker-but-honest fallback, it is a substantially *worse* correction — so "nothing decouples adequately → keep the plug-in" is a non-sequitur. And the two routes do not agree: ρ ≥ 0.80 on only 7/16, against the 12/16 the pre-registered rule required.
>
> This also kills the argument that had favoured branch 3, that the plug-in's depth-partialled residual is more reliable (0.976 vs 0.956). It is — but a large part of what remains in it is still depth, and depth is what reproduces under thinning. The same failure mode as the retired rank half of criterion 3.
>
> **The fourth branch, which the evidence actually selected:** Chao–Shen primary, plug-in co-reported, criterion 5(b) pre-registered as the gate on the one axis where Chao–Shen is worse. It is a **trade, not a clean win** — Chao–Shen removes most of the depth artefact and amplifies the `percent.mt` degradation artefact (spline-adjusted |r| 0.262 vs the plug-in's 0.222; worse on 12/16; positive on 14/16, which is a mechanism, not scatter). D2 carries the full argument, including the fair-comparison test that was run to see whether that finding would dissolve, and did not.
>
> **What this plan should have specified and didn't:** that A5 needed a *measurement*, not a paragraph. The three branches are exhaustive only if you already know which correction route works, which is the thing in question.

**Effort:** A1 half a day, A2 two hours, A3 half a day including the rewrite, A4 half a day, ~~A5 a paragraph~~ A5 a new diagnostic plus a 16-sample sweep, most of a day. Call it 2 days ~~→ closer to 3.~~
